...
- Not yet decided (proposed, awaiting Mitch): experiment queue, failure log, taster profiles, photo gallery on list cards, Year in Bakes, bake-day mode. Also pending: single-user auth (build before real data accumulates — next priority), feedback rollup stats on Insights tab, per-bake delete in the UI.
