...
- 2026-07-23: Bake flow overhaul (Emma + Mitch) — merged "Log the bake" + wrap-up into ONE page (/bakes/[bakeId]), now the canonical bake page (shown right after starting a bake AND the later edit screen). Version picker's "Bake this" (BakeThisButton, client, passes local date) → createBakeAction creates the bake immediately (weather + shareId) → combined page, so photos + QR are live from the start; "Save this Bake" (saveBakeAction) persists date/flour blend/notes/ratings. Deleted /bake/[recipeId]/log and wrap-up-form.tsx. TWO notes now: bakes.notes = "what I changed/did tonight", new bakes.outcome_notes = "how it turned out". REMOVED batch size and the whole bake-off feature (incl. the A/B taster form + f/actions bake-off branch); batchSize/isBakeoff/bakeoffDiff columns kept but deprecated (non-destructive). Categories changed to cookies/brownies/cakes/pies/snacks/muffins/other (enum migrated: cake→cakes, bites→snacks, +pies,+muffins; shared list in src/lib/categories.ts). KNOWN FOLLOW-UP: create-on-start means tapping "Bake this" then bailing leaves an empty bake, and there's still no per-bake delete in the UI — Mitch has 6 empty test bakes (dated 07-22) from the old flow to clean up; recommend adding a "Delete this bake" button.
- 2026-07-23: Recipe detail page tweaks (Emma + Mitch): (1) Source URL now renders as the full clickable link (break-all, own line) instead of a "source" label; category uses categoryLabel(). (2) VersionList Edit mode: the version title itself becomes an editable box pre-filled with the current name (renameVersionAction now nulls the label when it equals the auto "Version N" via a hidden autoName field); per-version delete kept. (3) Per-version "Bake this tonight" (BakeThisButton, label prop added) + "Tweak it first" buttons at the bottom of each version card (like the /bake picker); the page-bottom bake/tweak buttons were removed. (4) NEW: tap a version's name (normal mode) → /recipes/[id]/versions/[versionId]/edit — edits that version IN PLACE (updateVersionAction: label/ingredients/steps/diff/why), distinct from "Tweak it first" which forks a new version. Recipe-page "Tweak it first" now uses the default next (→ create version → bake → combined page), matching the picker; the old next=recipe "create version, stay" path is gone.
- 2026-07-23: Bake page layout (Mitch): ingredients & steps dropdown added at the top; flour blend stacked BELOW the date (not side-by-side); "Save this Bake" moved to the very bottom after photos + feedback (whole page is now one client component, BakePageClient, with the submit button linked to the form via form="bake-form"); added "Delete this bake" with a confirm step (deleteBakeAction also del()s photo blobs, then redirects home).
- 2026-07-23: App icon fixes (Mitch): the chocolate chips read as a frown, so the bottom three now curve upward (outer chips raised to y=284, middle lowest at y=296) = smile; both eyes equalized to r=13 at y=238 (were r=15 and r=10 at different heights). Re-render PNGs from the square SVG after any icon edit.
- 2026-07-23: Recipes page (Mitch): grouped under category headings in CATEGORIES order with per-group counts, alphabetical within each group (empty categories hidden); added a search box filtering on name + tags + category label with a clear button and no-match message. Row subtitle now shows GF type + tags (category is in the heading). Edit mode (rename/delete) still works inside groups.
- 2026-07-23: Recipe editing added (Mitch): "Edit recipe" button beside the title on the recipe page → /recipes/[id]/edit (updateRecipeAction) edits name/category/GF type/source URL/tags. Deliberately SEPARATE from version editing (tap a version name = ingredients/steps).
- 2026-07-23: Custom categories (Mitch): recipes.category converted from the recipe_category pgEnum to **free text** (migration: drop default → ALTER TYPE ... USING category::text → restore default 'other'; the unused enum type is left in the DB). Both the create and edit forms have the built-in dropdown PLUS an "…or type a new one" field (newCategory wins over the select; normalizeCategory() stores lowercase single-spaced, e.g. "quick breads"). categoryLabel() title-cases unknown values; orderedCategories() sorts built-ins first, then custom alphabetically, with "other" last. The edit page injects the recipe's current custom category as an extra <option> so editing other fields never loses it. Recipes list grouping + search work with custom categories.
- Not yet decided (proposed, awaiting Mitch): experiment queue, failure log, taster profiles, photo gallery on list cards, Year in Bakes, bake-day mode. Also pending: single-user auth (build before real data accumulates — next priority), feedback rollup stats on Insights tab, a way to rename/merge custom categories (typos create new groups).
