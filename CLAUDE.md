...
- 2026-07-20: Bake photo storage decided = **Vercel Blob** (Mitch confirmed; supersedes the tentative "likely Supabase Storage"). Photo upload shipped on the bake wrap-up (/bakes/[bakeId]): client downscales to ~1600px JPEG via canvas (EXIF-aware, normalizes HEIC) before a server action put()s to Blob and records the URL in the existing bake_photos table. next.config serverActions.bodySizeLimit raised to 4mb. Degrades gracefully when BLOB_READ_WRITE_TOKEN is unset (shows a setup notice). isBlobConfigured() in src/lib/blob.ts (a "use server" file can't export sync fns). Photos render in a 2-col gallery with per-photo delete (del() from Blob + row). SETUP PENDING: Mitch must create a Vercel Blob store (Storage → Create → Blob) to auto-inject BLOB_READ_WRITE_TOKEN; until then prod shows the notice. Thumbnails on bake list/history cards NOT added (out of scope for this request).
